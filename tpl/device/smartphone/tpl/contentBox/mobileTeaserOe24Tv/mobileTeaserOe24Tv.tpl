...
// // (db) 2018-02-15 - Artikel wieder reinnehmen
// // if ($isArticle) {
// //     return;
// // }
...
// (ws) 2018-02-15 Anforderung
// $showBox = $isValidLayout && ($isValidChannel | $isArticle)
// siehe auch tpl/device/smartphone/assets/vendor/facebook/facebookReferrerTvTeaser.tpl
...
$channelId = $channel->getId();
$isValidChannel = in_array($channelId, $validChannels) ? true : false;

// ------------

$configLayout = $device->getConfig('layout');
$isValidLayout = (in_array($configLayout, $layoutsBlacklist)) ? false : true;
...
// Box zeigen, wenn der Artikel KEIN Video enthaelt
...
if ($isArticle) {
    ...
    // Assoziierte Videos
    $relatedVideos = $configArticle->getRelatedVideos();
    $countRelatedVideos = count($relatedVideos);
    $hasNoVideo = ($countRelatedVideos > 0) ? ($hasNoVideo && false) : ($hasNoVideo && true);

    // Videos im Artikel-Body
    $pattern = '~<spunQ:Video id="\d*"/>~';
    $subject = $configArticle->getBodyText();
    $result = preg_match($pattern, $subject, $matches);
    $hasNoVideo = (0 === $result || false === $result) ? ($hasNoVideo && true) : ($hasNoVideo && false);
}

// ------------------------------------

// $showBox = $isValidLayout && ($isValidChannel | $isArticle);
$showBox = $isValidLayout && $hasNoVideo && ($isValidChannel | $isArticle);
