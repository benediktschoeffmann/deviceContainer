...
$options = ($isContentSet) ? $content->getOptions(true, true) : $channel->getOptions(true, true);
...
$adSlots = array();
foreach ($parentChannels as $pc) {
    $options = $pc->getOptions(false, true);
    $value = trim($options->get('AditionAdSlots') . "\n\r\t ");
    if ($value && $value != '') {
        $adSlots = $value;
        break;
    }
...
if ($adSlots) {
    $adSlotArray = array();
    $adSlotArray = json_decode($adSlots, true);

    if (is_array($adSlotArray)) {
        $adSlotsArrayFiltered = array_filter($adSlotArray, function ($e) use ($isContentSet) {

            $imChannelOk = (isset($e['channel']) && $e['channel'] && !$isContentSet);
            $imArtikelOk = (isset($e['artikel']) && $e['artikel'] && $isContentSet);
            $rc = preg_match('/(Mobile|Video)(.)*/i', $e['banner']);

            return ($imChannelOk || $imArtikelOk) && (0 == $rc);
        });

        $values = array_values($adSlotsArrayFiltered);
        $adSlots = json_encode($values);
    }
}
