...
// * @default oe2016layouts 1


// -------------------------------------------

// $contentParameter = request()->getGetValue('content');

// switch ($contentParameter) {

// 	case '2017':
// 	    etpl('oe24.oe24.__splitArea.content2017.articleDefault.articleDefault', array(
// 	        // 'portal' => $portal,
// 	        'channel' => $channel,
// 	        'params' => $params,
// 	        'object' => $object,
...
// (mh) DAILY-877 show all FB-Comments for Article 301173882
// (bs) DAILY-877 got updated - show 100 comments for all articles.
...
//MH 20170927 ShowAds global da öfters gebraucht
...
$showSocialConnectBox = ($articleOptions->get('hideSocialConnectBox')) ? false : true;
// -------------------------------------------
...
if ($pages > 1) {
	...
	// (bs) DAILY - 780 ist mal eine ziemlich russische Variante
	// AdPosition, die ausgespielt werden soll, muss noch definiert werden.

	$layoutOverride = $articleOptions->get('layoutOverride');

	if (1 && $showAds && ('oe24' == $layoutOverride)) {
		...
		foreach ($pTagsArray as $key => $pTag) {
			...
			if ($stringLengthSum > $limit && !$adAdded) {
				$adAdded = true;
				$modifiedBodyText .= '<div style="margin: 5px 0;>';
				$modifiedBodyText .=
...
// (db) 2017-04-27 Werbung anzeigen? hinzugefügt für Ligatus-Einbindung
$showLigatus = $showAds;
// showAds nur bei oe2016 anzeigen
if (true == $showLigatus) {
	$showLigatus = (in_array($layout, $oe2016layouts)) ? $showLigatus : false;
...
// (db) testartikel 279861923
// $showLigatus = ('279861923' == $articleId) ? true : false;
...
<div class="row <?= $recipeClass; ?>">
	...
	<div class="content article">
		...
		<article class="article_box">
			...
			<div class="article_box_wrapper">
				...
				<div class="article_body">
					...
					<? else : ?>
						<? $newsletterRegistration = (spunQ::inMode(spunQ::MODE_DEVELOPMENT)) ? 161625188 : 227144488; ?>
						<? if( $newsletterRegistration == $articleId): ?>
							<?
								// registrierungsformular
								etpl('oe24.oe24.__splitArea.newsletter.registerform', array('showDescription' => '1'));
							?>
						<? else: ?>
							<?=$articleBodyText;?>
						<? endif; ?>
			...
			<?
				if (in_array($layout, $oe2016layouts) && $showAds && $showSocialConnectBox) {
					// (db) 2017-08-29 box newsletter/facebook
					etpl('oe24.oe24.__splitArea.article.socialConnectBox', array(
						'channel' => $channel,
						'content' => $article,
						'layout'  => $layout,
					));
				...
				etpl('oe24.oe24.__splitArea.article.nationalratswahl2017Widget', array(
					'channel' => $channel,
					'content' => $article
				));
			?>
        ...
        <?
            ...
            if (in_array($layoutOverride, $allowedOverrides) && $showAds) {
		...
		// if ('gesund24' == $layout || ($showOutbrain && ($layoutOverride !== 'oe24'))) {
       ...
       if (in_array($layout, $oe2016layouts)) :
		...
		// etpl('oe24.oe24.adition.adSlot',array('channel' => $channel, 'id' => 'Left2'));
	...
	<div class="sidebar article">
		...
		<? if (in_array($layout, $oe2016layouts)): ?>
			<div class="sidebarContainer">
				...
				<?
					tpl('oe24.oe24.__splitArea.article.articleDetailSidebar', array(
						'channel' => $channel,
						'showAds' => $showAds,
						)
					);
				...
				<?
					tpl('oe24.oe24.adition.adSlot', array(
						'channel' => $channel,
						'id' => 'Contentad2'
						)
					);
...
if (in_array($layout, $oe2016layouts)) {
	foreach ($boxesFullWidth as $key => $box) {
		if ('oe24.oe24._templateBoxes.oe2016meinJobTopJobs' == $box->getTemplate()){
			tpl($box->getTemplate(), array(
				'box' 		=>	$box,
				'channel'	=>	$channel,
				'column'	=>	$channel->getColumnByName('Split-Story-Teaser Area'),
...
// (bs) 2018-01-17 Artikelkonsolidierung
// if (in_array($layout, $oe2016layouts)) {
