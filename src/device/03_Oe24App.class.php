...
class Oe24App {
    ...
    public function init($config = array()) {
        $this->config        = $config;
        // error(var_dump($config));
        $this->tplAnnotation = $this->getConfig('deviceType').'Tpl';
        $this->prepareAds();
        $this->articleHandler = new Oe24AppArticleHandler($this);
    }
    ...
    */
    ...
    public function processDevice() {
        ...
        if ($onlyArticles) {
            ...
            foreach ($categories as $key => $category) {
                ...
                // $isFrontpage = ($category['name'] === 'frontpage' || $category['name'] === 'sport') ? true : false;

                $isFrontpage = in_array($category['name'], $this->getConfig('maxBoxesSpecial'));
    ...
    private function getArticleDataById($id) {
        ...
        if ($article instanceof Video) {
            return $this->articleHandler->getVideoDataAsArticle($article);
        }
        return $this->articleHandler->getArticleData($article);
    }

    /**
     * returns video data
     * @param  oe24.core.Video $video [description]
     * @return array
     */
    /*
    ...
    */
    ...
    public function getMediaData($article) {
        return $this->articleHandler->getMediaData($article);
    }
    /*
    public function getMediaData($article) {
        $relatedData = array();
