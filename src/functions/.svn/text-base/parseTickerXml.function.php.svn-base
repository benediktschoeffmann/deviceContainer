...
function parseTickerXml($xml, $id, $sportIds) {
	 ...
	 if (array_key_exists($id, $sportIds)) {
		switch ($sportIds[$id]) {
			...
			case 'Skispringen':
				...
				break;
			default:
				$file = 'http://file.oe24.at/xml/laola1_olympia.xml';
				$xmlParseStyle = 2;
				break;
		}
		...
		$file = 'http://file.oe24.at/xml/laola1_olympia.xml';
		if (is_numeric($id)) {
			$sportartName = 'Olympiamix';
		} elseif ($id == 'Olympiamix' || $id == 'Konferenz') {
			$sportartName = $id;
		} else {
			$sportartName = 'Olympiamix';
		}
	...
	$xml = xmlCache::getXMLData($file,60);
	$tickerData = array();
	...
	if ($xmlParseStyle == 2) {
		...
		foreach ($sports as $sport) {
			if ($sport->attributes()->name->__toString() == $sportartName || ($sportartName=='Olympiamix' && $sport->attributes()->name->__toString() == 'Sportmix')) {
	...
	// (ws) 140207 - Zeitliche Reihenfolge der Tickereintraege stimmt nicht
